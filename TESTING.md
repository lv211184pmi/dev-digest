...
| mcp | `mcp/` | unit (stdio MCP server) | vitest | `mcp.yml` | no |
...
**mcp** — the stdio MCP server that exposes the review loop to MCP hosts: the
error catalogue, flat-argument resolution, poll-until-terminal, findings
projection, and an in-process SDK client asserting the five registered tools.
`fetch` against the API on `:3001` is stubbed; no API, DB, LLM, or Docker.
